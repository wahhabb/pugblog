...
p.
    But there are many situations where we do not want to get the same results every 
...
p. 
    ...
    is done by naming it the same as the page, but with the .js or .ts suffix.
p.
...
p.
    ...
    In your src directory, create a new file called about.js, and put 
    the following code in it:
<Prism language="javascript" code={k2j}  />
p.
    This is our endpoint. 
    What we are exporting here is a <i>request handler function</i> whose name would 
    be get, post, or whatever HTTP method it corresponds to. This will be 
    automatically invoked when the
    About page is navigated to. As a courtesy to the api developer, please update the
    User-Agent with a website or email address per the documentation.
p.
    Now we need to modify the About page to accept the return
    from this function. This will be passed to it as a prop, which is indicated by calling
    it an export on our page file. So replace the contents of our about.svelte with the
    following code:
<Prism language="javascript" code={k3j}  />
<Prism language="html" code={k3h}  />

p.
    At this point, navigating to your About page should display a random dad joke each time you
    navigate there.
p.
    There will be a slight lag when going to the About page. One thing we can do to speed this
