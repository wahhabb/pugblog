...
<\/script>`;
const d1h = `<div style = "font-size: { fontSize }">
	{ toDispString(val) }
</div>`;
const d1c = `<style>
	div {
...
<\/script>`;
const cb1h = `<button class="{width} {use}" >
    <slot></slot>
</button>`;
const cb1c = `<style>
	button {
...
const c1j = `<script>
	...
	let calc;
<\/script>`;
const c1h = `<div class="calc" style="--fontSize:{calcFontSize};" bind:this={calc}>
	<Display />
...
const s1j = `import { writable } from 'svelte/store';
...
const d2j = `	import { display } from './stores.js';
	let fontSize = "3.2em"
	const toDispString = (val) => {
		if (val == 0) return "0";
		return String(val);
...
`;
const d2h = `<div style = "font-size: { fontSize }">
	{ toDispString($display) }
</div>`;
const cb2h = `<button class="{width} {use}" on:click={(a) => calcClick(a)}>`;
const cb2m = `<script context="module">
	 let lastBtn = ""
	 let inDecimal = 0;
</scr\xECpt>`;
const cb2j = `	export let width = "";
	export let use = "number";
...
const d3j = `	import { display } from './stores.js';
	let rounded;
	let fontSize = "3.2em"
...
const cb3j = `			case "AC":
			...
			// });
				...
				break;`;
const cb4j = `		if ("0" <= btn && btn <= "9") {
			dispatch('ac', {
				symbol: btn,
			});`;
const c3j = `	const setClear = (event) => {
		let selected = event.detail.symbol;
		let opers = document.getElementsByClassName('fn');
		opers[0].innerHTML = 'C';
	};`;
const c4j = `  const setOperColor = (event) => {
		let selected = event.detail.symbol;
		if (selected === "=")
			selected = "";
